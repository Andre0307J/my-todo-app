...
import { addTodo, updateTodo } from "@/lib/todoApi.js";

// Define the shape of a Todo item for type safety across the component
type Todo = {
  id: number;
  todo: string;
  completed: boolean;
...
interface TodoFormProps {
  initialData?: Partial<Todo>; // `initialData` is optional and may not contain all Todo fields
...
export default function TodoForm({
  ...
  const addOrUpdateTodoMutation = useMutation<
    Todo, // Type of data returned by mutationFn
    Error, // Type of error
    Omit<Todo, "id"> // Type of variables passed to mutate()
  >({
    mutationFn: async (newOrUpdatedTodo: Omit<Todo, "id">) => {
      if (initialId) {
    ...
    onSuccess: (returnedApiData, submittedTodoData) => {
      ...
      queryClient.setQueryData<Todo[]>(["todos"], (oldData) => {
        // oldData is now typed as Todo[] | undefined
        ...
        if (initialId) {
          ...
          // queryClient.setQueryData(['todo', String(returnedApiData.id)], returnedApiData);
              ...
              queryClient.setQueryData(
                ["todo", String(returnedApiData.id)],
                returnedApiData
              );
      ...
      if (initialId) {
        const updatedItemForSingleView = {
          ...submittedTodoData,
          id: returnedApiData.id,
          completed: returnedApiData.completed,
        };
        queryClient.setQueryData(
          ["todo", String(initialId)],
          updatedItemForSingleView
        );
      }
    ...
    <form className={styles.form} onSubmit={handleSubmit}>
      ...
      <div className={styles.actions}>
        <button
          ...
          disabled={addOrUpdateTodoMutation.isPending}
        >
          {addOrUpdateTodoMutation.isPending
            ? initialId
              ? "Updating..."
              : "Creating..."
            : initialId
